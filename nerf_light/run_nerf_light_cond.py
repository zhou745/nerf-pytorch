...
def render(H, W, K, chunk=1024 * 32, rays_o=None, rays_d=None, viewdirs=None, light_cond=None,ref_img =None, device=None,
    ...
    if np.random.uniform(0.,1)<gt_light_rate:
        light_cond_cuda = light_cond.to(device)
    else:
        ref_img = ref_img.permute(0,3,1,2)
        light_cond_cuda = kwargs['model_light'](ref_img.to(device))
    # Render and reshape
    sh = rays_d.shape
...
def create_nerf(args):
    ...
    grad_vars += list(model_light.parameters())
    ...
    if args.N_importance > 0:
        ...
        # model_light_fine = vit_toy_patch16_256(num_classes=args.light_cond)

        grad_vars += list(model_density_fine.parameters())
        grad_vars += list(model_color_fine.parameters())
        # grad_vars += list(model_light_fine.parameters())
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        model_light.load_state_dict(ckpt['model_light'])
    ...
    for i in range(epoch_start, num_epoch):
        ...
        for data_batch in tqdm(dataloader_train):
            ...
            if global_step % args.print_freq_step == 0:
                tqdm.write(
                    f"[TRAIN] Iter: {global_step} Loss: {loss.item()}  PSNR: {psnr.item()}, Loss_fine: {img_loss.item()}, Loss_coarse: {img_loss0.item()}")
