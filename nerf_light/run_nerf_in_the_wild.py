...
def batchify(fn, num_sample,chunk):
    ...
    def ret(inputs):
        result_list = [fn({key: inputs[key][i:i+chunk] for key in inputs.keys()}) for i in range(0, num_sample, chunk)]
        result = {key:torch.cat([result_list[i][key] for i in range(len(result_list))], 0)  for key in result_list[0].keys()}
        return(result)
...
def render(H, W, K, chunk=1024 * 32, rays_o=None, rays_d = None, viewdirs = None,light_cond = None, device = None,
           near = 2.0, far = 6.0,eval_model=False, **kwargs):
    """Render rays
    Args:
    ...
    # Render and reshape
    sh = rays_d.shape
    all_ret = batchify_rays(rays_o_cuda,rays_d_cuda,viewdirs_cuda,light_cond_cuda, chunk,eval_model=eval_model, **kwargs)
    for k in all_ret:
...
def render_path(render_poses,light_cond, hwf, K, chunk, render_kwargs,img_idx=0, gt_imgs=None, savedir=None, render_factor=0):
    H, W, focal = hwf
    K_use = K
    if render_factor != 0:
...
def train():
    ...
    for i in range(epoch_start, num_epoch):
        ...
        for data_batch in tqdm(dataloader_train):
            ...
            rays_o = torch.stack([rays_o[i,select_coords[i,:, 0], select_coords[i,:, 1]] for i in range(N)],dim=0)  # (N_rand, 3)
            rays_d = torch.stack([rays_d[i,select_coords[i,:, 0], select_coords[i,:, 1]] for i in range(N)],dim=0)  # (N_rand, 3)
            viewdirs = rays_d/rays_d.norm(dim=-1,keepdim=True)
            target_s = torch.stack([images[i,select_coords[i,:, 0], select_coords[i,:, 1]] for i in range(N)],dim=0)  # (N_rand, 3)
            ...
            if global_step % args.print_freq_step == 0:
                tqdm.write(f"[TRAIN] Iter: {global_step} Loss: {loss.item()}  PSNR: {psnr.item()}, Loss_fine: {img_loss.item()}, Loss_coarse: {img_loss0.item()}")
