...
def train():
    ...
    for i in range(epoch_start, num_epoch):
        ...
        if epoch_step % args.save_testset_freq_epoch == 0 and epoch_step > 0:
            ...
            render_dataset(testsavedir, hwf, K_ori, args, dataset_test, render_kwargs_test, device,
                           offset_idx=0, step_idx=1, num_render=20, light_cond_ratio=None, gt_light_rate=1.1)
...
def render_dataset(save_dir, hwf, K_ori, args, dataset, render_kwargs_test, device, offset_idx=0, step_idx = 1,
    ...
    with torch.no_grad():
        for img_idx in tqdm(range(min(num_render,len(dataset)))):
            ...
            rgb_opt,_ =render_path(poses, light_cond, hwf, K, args.chunk, render_kwargs_test, img_idx=2*img_idx+1,
                        gt_imgs=images,savedir=testsavedir, render_factor=render_factor,
                        gt_light_rate= gt_light_rate)
            N,H,W,_ = images.shape
            rgb = rgb.reshape(N,H,W,-1)
            rgb_opt = rgb_opt.reshape(N,H,W,-1)
            loss += img2mse(rgb[valide_mask], images[valide_mask])
            loss_opt += img2mse(rgb_opt[valide_mask], images[valide_mask])
    loss = loss/min(num_render,len(dataset))
    loss_opt = loss_opt/min(num_render,len(dataset))
    psnr = mse2psnr(loss)
    psnr_opt = mse2psnr(loss_opt)
    print(f'final loss val loss is {loss.item()} psnr val is {psnr.item()}')
    print(f'final loss_opt val loss_opt is {loss_opt.item()} psnr_opt val is {psnr_opt.item()}')
    print('Saved test set')
